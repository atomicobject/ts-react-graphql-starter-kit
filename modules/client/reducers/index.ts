...
export function gameReducer(state: GameState, action: ActionTypes): GameState {
  ...
  case ActionTypes.ANSWER_CHANGED:
    return flow(
      GameState.answerSequence.set(action.answer),
    )(state)

  case ActionTypes.GOOD_GUESS_OCCURRED:
    return flow(
      GameState.gameWon.set(false),
      GameState.lastGuess.set({status: GuessResult.CORRECT, value: action.value}),
  ...
  case ActionTypes.BAD_GUESS_OCCURRED:
    return flow(
      GameState.gameWon.set(false),
      GameState.lastGuess.set({status: GuessResult.INCORRECT, value: action.value}),
...
function targetReducer<T,U>(reducer: (arg:U, action: any) => U, lens: Lens<T,U>): (arg:T, action: any) => T {
